...
import { 
  ...
  Star, Zap, Anchor, Plane, Building2, CheckCircle2, ChevronRight
} from 'lucide-react';
...
export const HowItWorks: React.FC = () => {
    ...
    { 
      ...
      size: 'large',
      image: 'https://plus.unsplash.com/premium_photo-1723527890050-0b688370e70f?w=1600&auto=format&fit=crop&q=80'
    },
    { 
      title: 'Ocean Might', 
      icon: Anchor, 
      ...
      size: 'small',
      image: 'https://images.unsplash.com/photo-1494412519320-aa613dfb7738?auto=format&fit=crop&q=80&w=800&h=800'
    },
    { 
      title: 'Road Network', 
      icon: Truck, 
      ...
      size: 'small',
      image: 'https://images.unsplash.com/photo-1519003722824-194d4455a60c?auto=format&fit=crop&q=80&w=800&h=800'
    },
    { 
          ...
          {solutions.map((item, idx) => (
            ...
            >
              <div className="relative z-20 h-full flex flex-col">
                <div className="w-16 h-16 bg-white/95 backdrop-blur-md rounded-2xl flex items-center justify-center text-primary shadow-xl group-hover:bg-primary group-hover:text-white transition-all duration-500 mb-8 shrink-0 flex-none">
                  <item.icon className="w-8 h-8 shrink-0 flex-none" />
                </div>
                <h3 className="text-2xl font-display font-black text-navy mb-4 uppercase tracking-tight group-hover:text-primary transition-colors drop-shadow-sm">{item.title}</h3>
                <p className="text-gray-600 font-bold leading-relaxed mb-6 group-hover:text-navy transition-all drop-shadow-sm">{item.desc}</p>
                        ...
                        <div key={subIdx} className="bg-white rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-14 shadow-2xl border border-gray-50 flex flex-col justify-between h-full group hover:shadow-primary/5 transition-all duration-500 relative">
                           ...
                           <div className="relative z-10">
                              <div className="flex gap-1 mb-6 md:mb-8 shrink-0">
                                {[...Array(item.stars)].map((_, i) => <Star key={i} className="w-3 md:w-4 h-3 md:h-4 text-primary fill-primary shrink-0" />)}
                              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-12">
          {stats.map((stat, idx) => (
            ...
            >
              <stat.icon className="w-8 md:w-10 h-8 md:h-10 text-primary mb-6 md:mb-8 group-hover:scale-110 transition-transform duration-500 shrink-0" />
