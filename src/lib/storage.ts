...
const HISTORY_KEY = 'swifttrack_history';
...
export const getTrackedHistory = (): string[] => {
...
export const addToHistory = (trackingId: string): void => {
  const history = getTrackedHistory();
  const filtered = history.filter(id => id.toUpperCase() !== trackingId.toUpperCase());
  const newHistory = [trackingId.toUpperCase(), ...filtered].slice(0, 10); // Keep last 10
